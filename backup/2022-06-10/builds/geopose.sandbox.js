...
/** Defines a Font Resource. */
	 ...
	 * @param data The font JSON data.
	 * @returns The Font Resource. */
	static parse(data) {
		let loader = new THREE.FontLoader();
...
let defaultFontData = {
	...
	"glyphs": {
       ...
       let defaultFont = FontResource.parse(defaultFontData);
...
/** Defines an User Interaction Viewport. */
	...
	get spaces() { return this._layers; }
	...
	link(space) {
		...
		if (this._layers.indexOf(space) >= 0)
	...
	unlink(space) {
		...
		if (spaceIndex >= 0)
			this._layers = this._layers.splice(spaceIndex, 1);
		else
 ...
 * @returns The generated element. */
       function createDomElement(type, id, classes, parent, content, style) {
...
/** Defines a Text Interaction Component. */
	...
	* @param deltaTime The update time. */
	update(forced = false, deltaTime = 0) {
		...
		if (!this.text.updated)
			text.characters.value = this.text.value;
...
/** Defines a Heads-Up Display Layer. */
       class HeadsUpDisplayLayer extends Layer {
}
