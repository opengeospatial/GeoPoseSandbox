...
export class GeoPosition extends Position {
	...
	private _tangentVector: Vector;

	/** The vertical vector of the GeoPosition. */
	...
	update(deltaTime: number = 0, forced: boolean = false) {
		...
		let geoFrame: GeoFrame = (this.parent as GeoPose).frame,
			equatorRadius = geoFrame.equatorialRadius.value,
			polarRadius = geoFrame.polarRadius.value,
			flatteningFactor = polarRadius / equatorRadius;

		// Perform some basic trigonometric calculations
		let lng = -this.longitude.value  * (Math.PI/180), 
			lat = this.latitude.value * (Math.PI/180),
			lngSin = Math.sin(lng), lngCos = Math.cos(lng),
			latSin = Math.sin(lat), latCos = Math.cos(lat),
			alt = this.altitude.value,
			geoX = lngCos * latCos, geoY = latSin, geoZ = lngSin * latCos;
		...
		let v: any = this._verticalVector.getValues();

		// Calculate the tangent vector
		this.relativeValues.setValues(x+v.x*alt, y+v.y*alt, z+v.z*alt);	

		// Calculate the tangent vector
		let x0 = latSin * equatorRadius, x1 = latSin * (equatorRadius + 1), 
			y0 = latCos * equatorRadius * flatteningFactor,
			y1 = latCos * (equatorRadius + 1) * flatteningFactor,
			dx = x1 - x0, dy = y1 - y0, l = Math.sqrt((dx * dx) + (dy * dy));
