...
export class Viewport {
	...
	link(space: Space) {
		...
		if(this._spaces.indexOf(space) >= 0) throw Error(
		...
		// Link the subspaces
		for (let subspace of space.spaces.childNodes) this.link(subspace as Space);
	...
	unlink(space:Space) {
		...
		if (spaceIndex >= 0) this._spaces = this._spaces.splice(spaceIndex, 1);
